...
运行时把每张整帧固定绘制在同一矩形（底部居中锚点），不做任何逐帧位移补偿。因此角色在屏幕上的稳定性完全依赖帧图片本身的位置一致性：`tools/prepare_frame_animation_v2.py` 在切割生成表后，会把每个中间帧按脚部锚点水平对齐到 `neutral_512.png`，待机帧同时对齐底边。绕过该脚本直接修改帧文件会重新引入晃动。对齐细节见 `docs/ASSETS.md`。
