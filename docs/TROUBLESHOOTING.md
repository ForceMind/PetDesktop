...
确认运行的是 v1.5.0 或更新版本，并重新执行 `build.ps1 -Clean`。新版一次只绘制一张完整帧，不应出现交叉淡入。若仍闪烁，运行 `py tools/test_authored_frames.py` 检查帧数量和精确端点。
...
发布包使用临时本地签名。首次运行可在 Finder 中右键 `Coco桌宠.app` 并选择“打开”。若提示文件损坏，请确认 ZIP 已完整下载，并从 GitHub Release 重新获取。
