...
运行日志，也可在独立“测试数据”页签按匿名浏览器查看限期保存的测试对话，或一键在服务器端
关闭新的聊天、AI 和游戏请求。
...
已有 Node/npm。多发行版、首次切换分支、HTTPS、日志和回滚说明见
...
- Web 版额外提供 32 动作点播、自动表演/待机/对白开关、场景背景、全屏、暂停、快捷键、设置记忆，并在启动后于后台预取和缓存全部动作帧；手机端支持底部控制抽屉、触摸拖动、点击部位和双指缩放。
...
- [AI 游戏服务器部署](docs/AI_GAME_SERVER_DEPLOYMENT.md)
