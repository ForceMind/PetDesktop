...
const gameIg = (process.env.GAME_BINGO_IG || process.env.GAME_SLOT_IG || "").trim();
const timeoutMs = Number(process.env.GAME_REQUEST_TIMEOUT_MS || 10_000);
const allowOnePlay = process.env.BINGO_ALLOW_ONE_PLAY === "1";

if (origin !== TEST_ORIGIN) {
  throw new Error(`Refusing Bingo diagnostics outside ${TEST_ORIGIN}.`);
}
if (apiBaseUrl !== TEST_API_BASE_URL) {
  throw new Error(`Refusing Bingo diagnostics outside ${TEST_API_BASE_URL}.`);
}
if (!gameIg) throw new Error("GAME_BINGO_IG or GAME_SLOT_IG is required.");
...
const initial = await post("/game3/bingo/init", context, false);
const initialData = requireData(initial, "Bingo init");
const balanceBefore = finiteNonNegative(initialData.token, "Bingo init token");
const bets = requireNumberArray(initialData.bets, "Bingo init bets");
...
if (!allowOnePlay) {
  process.stdout.write(`${JSON.stringify({
    ...
    operation: "init-only",
    init: { code: initial.code, balance: balanceBefore, bets }
  }, null, 2)}\n`);
  process.exit(0);
}
...
try {
  playSent = true;
  rawPlay = await post("/game3/bingo/play", {
    token: minimumBet,
    type: "",
    auto: 0,
    card: 4,
...
const finalInit = await post("/game3/bingo/init", context, false);
const finalData = requireData(finalInit, "Bingo final init");
const balanceAfter = finiteNonNegative(finalData.token, "Bingo final init token");
process.stdout.write(`${JSON.stringify({
  environment: TEST_ORIGIN,
  operation: "one-lowest-bet-play",
  playSent,
...
async function safeInit(body) {
  try {
    return await post("/game3/bingo/init", body, false);
  } catch {
    return undefined;
  }
}
...
async function post(path, body, outcomeCanBeUnknown) {
  ...
  try {
    ...
    if (outcomeCanBeUnknown) {
      throw new Error(
        `Bingo Play outcome is unknown (${error instanceof Error ? error.message : "request failure"}); do not retry.`
      );
    }
    throw error;
  } finally {
