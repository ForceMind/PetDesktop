...
const gameIg = (process.env.GAME_CHARMED_IG || process.env.GAME_SLOT_IG || "").trim();
const timeoutMs = Number(process.env.GAME_REQUEST_TIMEOUT_MS || 10_000);
const allowOnePlay = process.env.CHARMED_ALLOW_ONE_PLAY === "1";

if (origin !== TEST_ORIGIN) {
  throw new Error(`Refusing Charmed diagnostics outside ${TEST_ORIGIN}.`);
}
if (apiBaseUrl !== TEST_API_BASE_URL) {
  throw new Error(`Refusing Charmed diagnostics outside ${TEST_API_BASE_URL}.`);
}
if (!gameIg) throw new Error("GAME_CHARMED_IG or GAME_SLOT_IG is required.");
...
const initial = await post("/game3/charmed/init", context, false);
const initialData = requireData(initial, "Charmed init");
const balanceBefore = finiteNonNegative(initialData.token, "Charmed init token");
const bets = requireNumberArray(initialData.bets, "Charmed init bets");
...
if (minimumBet !== 10) {
  throw new Error(`Refusing Play because the observed minimum Charmed bet is ${minimumBet}, not 10.`);
}
...
try {
  playSent = true;
  rawPlay = await post("/game3/charmed/play", {
    token: minimumBet,
    type: "",
    auto: 0,
    ...context
...
const finalInit = await post("/game3/charmed/init", context, false);
const finalData = requireData(finalInit, "Charmed final init");
const balanceAfter = finiteNonNegative(finalData.token, "Charmed final init token");
process.stdout.write(`${JSON.stringify({
  environment: TEST_ORIGIN,
  apiBaseUrl: TEST_API_BASE_URL,
  operation: "one-lowest-bet-play",
...
async function safeInit(body) {
  try {
    return await post("/game3/charmed/init", body, false);
  } catch {
    return undefined;
  }
}
...
async function post(path, body, outcomeCanBeUnknown) {
  ...
  try {
    ...
    if (outcomeCanBeUnknown) {
      throw new Error(
        `Charmed Play outcome is unknown (${error instanceof Error ? error.message : "request failure"}); do not retry.`
      );
    }
    throw error;
  } finally {
...
function summarizeJson(value, depth) {
  ...
  if (value && typeof value === "object") {
    return Object.fromEntries(
      Object.entries(value).map(([key, entry]) => [key, summarizeJson(entry, depth + 1)])
    );
  }
  return `<${typeof value}>`;
}
